...
def generate_nfsm_from_pregexp(pregexp: List[str], alphabet: List) -> NKA:
    """
    alphabet - Допустимый алфавит
    pregexp - Постфиксная регулярка
    Возвращает: Начальное состояние НКА
    """
    stack = []
    while len(pregexp) > 0:
...
class DFSMState:
    ...
    def __init__(self, state: str, nka_states: Set[str], outputs: List[Tuple[str, str]] = None, is_final=False):
        """
        state - Состояние
        nka_states - Множество состояний НКА, соответствующих данному состоянию ДКА
        outputs - Переходы в другие состояния в формате (state, symbol)
        is_final - Является ли состояние финальным
        """
        self.state = state
...
def __eps_closure_for_nka_state(nfsm, state: str) -> Set[str]:
    ...
    nfsm - НКА
    state - Состояние
    Возвращает: Множество состояний, достижимых из данного только по eps-переходам
    """
    ans = {state}
    stack = [state]
    was_seen = []
...
def __eps_closure_for_set_of_nka_states(nfsm, states) -> Set[str]:
    ...
    nfsm - НКА
    states - Состояния
    Возвращает: Множество состояний, достижимых из данного множества состояний только по eps-переходам
    """
    ans = set()
    for state in states:
...
def __move_closure_for_nka_state(nfsm, state: str, symbol: str) -> Set[str]:
    ...
    state - Состояние
    symbol - Символ
    Возвращает: Множество состояний, достижимых из данного по символу symbol
    """
...
def __move_closure_for_set_of_nka_states(nfsm, states, symbol: str) -> Set[str]:
    ...
    states - Состояния
    symbol - Символ
    Возвращает: Множество состояний, достижимых из данного множества состояний по символу symbol
    """
    ans = set()
    for state in states:
...
class MinDFSMState:
    ...
    def __init__(self, state: str, dka_states: Set[DFSMState], outputs: List[Tuple[str, str]] = None, is_final=False):
        """
        state - Состояние
        dka_states - Множество состояний ДКА, соответствующих данному состоянию минимального ДКА
        outputs - Переходы в другие состояния в формате (state, symbol)
        is_final - Является ли состояние финальным
        """
        self.state = state
...
def __find_finals_and_rest(dka: Iterable[DFSMState]) -> Tuple[Set[DFSMState], Set[DFSMState]]:
    """
    dka - ДКА
    Возвращает: Списки финальных и нефинальных состояний
    """
    fin = set()
    non_fin = set()
    for state in dka:
...
def __find_all_inputs_for_state(dka: List[DFSMState], dka_state: DFSMState, sym: str) -> Set[DFSMState]:
    """
    dka - ДКА
    dka_state - Состояние, входы в которое мы ищем
    sym - По данному символу
    Возвращает: Названия входящих состояний
    """
    ans = set()
    for state in dka:
